#!/usr/bin/env bash
#
# quickstart_philo.sh — run the full intel pipeline end-to-end for the PHILO
# streaming competitive set. Fully isolated from the Bobs and TREX deployments.
#
# Isolation:
#   - INTEL_DB_PATH          → data/philo.db
#   - INTEL_DATA_DIR         → data/philo_assets/
#   - INTEL_COMPETITORS_FILE → config/competitors_philo.yaml
#   - reports                → reports/philo/<UTC-date>/
#
...
#
# Usage:
#   ./quickstart_philo.sh                  # full pipeline (ingest + reports)
#   ./quickstart_philo.sh --skip-ingest    # use existing data, just rebuild reports
#   ./quickstart_philo.sh --reports-only   # alias for --skip-ingest
#   ./quickstart_philo.sh --days 14        # widen the analysis window (default 7)
...
export INTEL_DB_PATH="$ROOT/data/philo.db"
export INTEL_DATA_DIR="$ROOT/data/philo_assets"
export INTEL_COMPETITORS_FILE="$ROOT/config/competitors_philo.yaml"
...
# ---- preflight ----
bold "[1/10] preflight (PHILO deployment)"
rule
...
# ---- output dir ----
DATE="$(date -u +%Y-%m-%d)"
REPORTS="reports/philo/${DATE}"
mkdir -p "$REPORTS"
green "  ✓ writing reports to: $REPORTS/"
echo

# ---- 2. sync db from S3 ----
# data/philo.db is no longer git-tracked (moved to S3, see S3_ASSETS.md) — on
...
# ---- 3. init ----
bold "[3/10] init db (philo.db)"; rule
.venv/bin/intel init
echo
...
echo "    open $REPORTS/by-brand/philo.md"
