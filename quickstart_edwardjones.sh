...
# ---- preflight ----
bold "[1/10] preflight (EDWARD JONES deployment)"
rule
...
# ---- 2. sync db from S3 ----
# data/edwardjones.db is no longer git-tracked (moved to S3, see
# S3_ASSETS.md) — on a machine that's never run this before, or one that's
# behind another operator's last audit run, this pulls the latest before
# init/ingest touch it. Skips gracefully (not a hard fail) if S3 isn't
# configured, so a local-only/offline run still works, same spirit as the
# ANTHROPIC/META token checks above.
...
# ---- 3. init ----
bold "[3/10] init db (edwardjones.db)"; rule
.venv/bin/intel init
echo
