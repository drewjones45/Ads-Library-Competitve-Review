...
# ---- preflight ----
bold "[1/10] preflight (JDSPORTS deployment)"
rule
...
# ---- 2. sync db from S3 ----
# data/jdsports.db is no longer git-tracked (moved to S3, see S3_ASSETS.md) —
# on a machine that's never run this before, or one that's behind another
...
# ---- 3. init ----
bold "[3/10] init db (jdsports.db)"; rule
.venv/bin/intel init
echo
